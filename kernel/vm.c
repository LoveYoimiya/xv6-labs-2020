...
u2kvmcopy(pagetable_t pagetable, pagetable_t kernelpt, uint64 oldsz, uint64 newsz)
{
  ...
  for (uint64 i = oldsz; i < newsz; i += PGSIZE) {
    // get the physical address of the virtual address(i) in user pagetable
    if ((pte_from = walk(pagetable, i, 0)) == 0) 
      panic("u2kvmcopy: src pte does not exist");
    // 1 is alloc mode.it will alloc one page for the kernel pagetable
    if ((pte_to = walk(kernelpt, i, 1)) == 0) 
      panic("u2kvmcopy: pte walk failed");
    uint64 pa = PTE2PA(*pte_from);
    // in kernel mode, it's not allowed to visit the page set to PTE_U
...
/**
 * @param pagetable : the pagetabel that will be printed
 * @param level : the level of the pagetable now
*/
void 
...
/**
 * @brief print the pagetable by level
 * @param pagetable : the first level pagetable
*/
void 
